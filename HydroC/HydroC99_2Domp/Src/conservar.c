...
void gatherConservativeVars(const int idim, const int rowcol, const int Himin, const int Himax,
                            const int Hjmin, const int Hjmax, const int Hnvar, const int Hnxt,
                            const int Hnyt, const int Hnxyt, const int slices, const int Hstep,
...
#ifdef TARGETON
#pragma omp target map(u [0:Hnvar] [0:Hstep] [0:Hnxyt]) map(uold [0:Hnvar * Hnxt * Hnyt])
#endif
#ifdef LOOPFORM
#pragma omp teams loop bind(teams) private(s, i), collapse(2)
#else
#pragma omp TEAMSDIS parallel for default(none) private(s, i),                                     \
   FIRSTP(slices, Himin, Himax, rowcol, Hnxt, Hnyt, Hnxyt, Hstep) shared(u, uold) collapse(2)
#endif
        for (s = 0; s < slices; s++) {
...
#pragma omp TEAMSDIS parallel for default(none)                                                    \
    FIRSTP(Hnvar, slices, Himin, Himax, rowcol, Hnxt, Hnyt, Hnxyt, Hstep),                                 \
    private(s, i, ivar), shared(u, uold) collapse(3)
        ...
        // Gather conservative variables
#ifdef TARGETON
#pragma omp target map(u [0:Hnvar] [0:Hstep] [Himin:Himax]) map(uold [0:Hnvar * Hnxt * Hnyt])
#endif
#ifdef LOOPFORM
#pragma omp teams loop bind(teams) private(s, j), collapse(2)
#else
#pragma omp TEAMSDIS parallel for default(none)                                                    \
    FIRSTP(Hnvar, slices, Hjmin, Hjmax, rowcol, Hnxt, Hnyt, Hnxyt, Hstep),                                 \
...
void updateConservativeVars(const int idim, const int rowcol, const real_t dtdx, const int Himin,
                            const int Himax, const int Hjmin, const int Hjmax, const int Hnvar,
                            const int Hnxt, const int Hnyt, const int Hnxyt, const int slices,
                            const int Hstep, real_t uold[Hnvar * Hnxt * Hnyt],
...
#pragma omp teams loop bind(teams) private(s, i, ivar), collapse(3)
#else
#pragma omp TEAMSDIS parallel for FIRSTP(slices, Himin, Himax, rowcol, dtdx, Hnxt,           \
...
#else
#pragma omp TEAMSDIS parallel for default(none)                                                    \
    FIRSTP(rowcol, Hnvar, slices, Himin, Himax, dtdx, Hnxt, Hnyt, Hnxyt, Hstep),                           \
...
#pragma omp TEAMSDIS parallel for default(none) private(s, j, ivar),                               \
    FIRSTP(slices, Hjmin, Hjmax, dtdx, Hnxt, Hnyt, rowcol, Hnxyt, Hstep) shared(u, uold, flux) collapse(2)
#endif
        for (s = 0; s < slices; s++) {
            for (j = (Hjmin + ExtraLayer); j < (Hjmax - ExtraLayer); j++) {
                uold[IHU(rowcol + s, j, ID)] =
                    u[ID][s][j] + (flux[ID][s][j - 2] - flux[ID][s][j - 1]) * dtdx;
                uold[IHU(rowcol + s, j, IV)] =
                    u[IU][s][j] + (flux[IU][s][j - 2] - flux[IU][s][j - 1]) * dtdx;
                uold[IHU(rowcol + s, j, IU)] =
                    u[IV][s][j] + (flux[IV][s][j - 2] - flux[IV][s][j - 1]) * dtdx;
                uold[IHU(rowcol + s, j, IP)] =
                    u[IP][s][j] + (flux[IP][s][j - 2] - flux[IP][s][j - 1]) * dtdx;
            }
        }
        {
...
#else
#pragma omp TEAMSDIS parallel for default(none)                                                    \
    FIRSTP(Hnvar, slices, Hjmin, Hjmax, dtdx, Hnxt, Hnyt, rowcol, Hnxyt, Hstep), \
                ...
                for (s = 0; s < slices; s++) {
                    for (j = Hjmin + ExtraLayer; j < Hjmax - ExtraLayer; j++) {
                        uold[IHU(rowcol + s, j, ivar)] =
                            u[ivar][s][j] + (flux[ivar][s][j - 2] - flux[ivar][s][j - 1]) * dtdx;
                    }
                }
            }
        }
