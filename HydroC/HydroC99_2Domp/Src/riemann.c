...
#ifdef WITHTARGET
#pragma omp target map(qleft [0:Hnvar] [0:Hstep] [0:Hnxyt])                                        \
    map(qright [0:Hnvar] [0:Hstep] [0:Hnxyt]),                                                     \
    map(qgdnv [0:Hnvar] [0:Hstep] [0:Hnxyt]),                                                      \
    map(alloc                                                                                      \
        : sgnm [0:Hstep] [0:Hnxyt]),                                                               \
    map(pstar [0:tmpsiz]), map(rl [0:tmpsiz]), map(ul [0:tmpsiz]), map(pl [0:tmpsiz]),             \
    map(rr [0:tmpsiz]), map(ur [0:tmpsiz]), map(cl [0:tmpsiz]), map(pr [0:tmpsiz]),                \
    map(cr [0:tmpsiz]), map(goon [0:tmpsiz])
#endif
#ifdef LOOPFORM
...
#pragma omp TEAMSDIS parallel for default(none) private(s, i),                                     \
    ...
    for (s = 0; s < slices; s++) {
        ...
        for (i = 0; i < narray; i++) {
            ...
            pstar[ii] = FMAX(((wr_i * pl[ii] + wl_i * pr[ii]) + wl_i * wr_i * (ul[ii] - ur[ii])) /
                                 (wl_i + wr_i),
                             0.0);
            goon[ii] = 1;
        }
...
#pragma omp target map(pstar [0:tmpsiz]), map(rl [0:tmpsiz]), map(ul [0:tmpsiz]),                  \
    map(pl [0:tmpsiz]), map(rr [0:tmpsiz]), map(ur [0:tmpsiz]), map(cl [0:tmpsiz]),                \
    map(pr [0:tmpsiz]), map(cr [0:tmpsiz]), map(goon [0:tmpsiz])
#endif
#ifdef LOOPFORM
        ...
        for (i = 0; i < narray; i++) {
            ...
            for (iter = 0; iter < Hniter_riemann; iter++) {
                ...
                if (goon[ii]) {
                    ...
                    real_t wwl = (real_t)SQRT(cl[ii] * (one + gamma6 * (pst - pl[ii]) / pl[ii]));
                    real_t wwr = (real_t)SQRT(cr[ii] * (one + gamma6 * (pst - pr[ii]) / pr[ii]));
                    real_t swwl = Square(wwl);
                    real_t ql = two * wwl * swwl / (swwl + cl[ii]);
                    real_t qr = two * wwr * Square(wwr) / (Square(wwr) + cr[ii]);
...
#pragma omp target map(qleft [0:Hnvar] [0:Hstep] [0:narray])                                       \
    map(qright [0:Hnvar] [0:Hstep] [0:narray]),                                                    \
    map(qgdnv [0:Hnvar] [0:Hstep] [0:narray]), map(sgnm [0:Hstep] [0:Hnxyt]),                      \
    map(pstar [0:tmpsiz]), map(rl [0:tmpsiz]), map(ul [0:tmpsiz]), map(pl [0:tmpsiz]),             \
    map(rr [0:tmpsiz]), map(ur [0:tmpsiz]), map(cl [0:tmpsiz]), map(pr [0:tmpsiz]),                \
    map(cr [0:tmpsiz]), map(goon [0:tmpsiz]) //
#endif
...
#pragma omp TEAMSDIS parallel for default(none) private(s, i),                                     \
    ...
    for (s = 0; s < slices; s++) {
        for (i = 0; i < narray; i++) {
            ...
            wr_i = SQRT(cr[ii] * (one + gamma6 * (pstar[ii] - pr[ii]) / pr[ii]));
            wl_i = SQRT(cl[ii] * (one + gamma6 * (pstar[ii] - pl[ii]) / pl[ii]));

            real_t ustar_i = half * (ul[ii] + (pl[ii] - pstar[ii]) / wl_i + ur[ii] -
                                     (pr[ii] - pstar[ii]) / wr_i);
            ...
            real_t rstar_i = ro_i / (one + ro_i * (po_i - pstar[ii]) / Square(wo_i));
            ...
            real_t scr_i =
                FMAX((real_t)(spout_i - spin_i), (real_t)(Hsmallc + FABS(spout_i + spin_i)));
...
#ifdef WITHTARGET
#pragma omp target map(qleft [0:Hnvar] [0:Hstep] [0:narray]),                                      \
    map(qright [0:Hnvar] [0:Hstep] [0:narray]), map(qgdnv [0:Hnvar] [0:Hstep] [0:narray]),         \
