...
void hydro_godunov(int idimStart, real_t dt, const hydroparam_t H, hydrovar_t *Hv, hydrowork_t *Hw,
                   hydrovarwork_t *Hvw) {
    // Local variables
...
// #pragma omp target update to (uold [0:H.nvar *H.nxt * H.nyt])
        ...
        for (j = Hmin; j < Hmax; j += Hstep) {
            ...
            {
                gatherConservativeVars(idim, j, H.imin, H.imax, H.jmin, H.jmax, H.nvar, H.nxt,
                                       H.nyt, H.nxyt, slices, Hstep, uold, u);
                // Convert to primitive variables
                constoprim(Hdimsize, H.nxyt, H.nvar, H.smallr, slices, Hstep, u, q, e);
                equation_of_state(0, Hdimsize, H.nxyt, H.nvar, H.smallc, H.gamma, slices, Hstep, e,
                                  q, c);
                ...
                //
                start = cclock();
                qleftright(idim, H.nx, H.ny, H.nxyt, H.nvar, slices, Hstep, qxm, qxp, qleft,
                           qright);
                end = cclock();
                //
                functim[TIM_QLEFTR] += ccelaps(start, cclock());
                riemann(Hndim_1, H.smallr, H.smallc, H.gamma, H.niter_riemann, H.nvar, H.nxyt,
                        slices, Hstep, qleft, qright, qgdnv, sgnm, Hw);
                cmpflx(Hdimsize, H.nxyt, H.nvar, H.gamma, slices, Hstep, qgdnv, flux);
                updateConservativeVars(idim, j, dtdx, H.imin, H.imax, H.jmin, H.jmax, H.nvar, H.nxt,
                                       H.nyt, H.nxyt, slices, Hstep, uold, u, flux);
            }
        } // for j
    }
#ifdef TRACKDATA
