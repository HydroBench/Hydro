...
void trace(const real_t dtdx, const int n, const int Hscheme, const int Hnvar, const int Hnxyt,
...
#ifdef TARGETON
    //
#pragma omp target map(c [0:Hstep] [0:Hnxyt]) map(q [0:Hnvar] [0:Hstep] [0:Hnxyt])                 \
    map(dq [0:Hnvar] [0:Hstep] [0:Hnxyt]) map(qxp [0:Hnvar] [0:Hstep] [0:Hnxyt])                   \
        map(qxm [0:Hnvar] [0:Hstep] [0:Hnxyt])
#endif

    // This loop form has been temporarly deactivated probably because of a bug in icx 2021.1
    // version
#ifdef LOOPFORM_off
