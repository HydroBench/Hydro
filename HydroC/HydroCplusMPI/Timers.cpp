...
void Timers::getStats(void)
{
	...
	MPI_Allreduce(&m_elaps[f], &lvmin, 1, MPI_DOUBLE, MPI_MIN,
		      MPI_COMM_WORLD);
	MPI_Allreduce(&m_elaps[f], &lvmax, 1, MPI_DOUBLE, MPI_MAX,
		      MPI_COMM_WORLD);
	MPI_Allreduce(&m_elaps[f], &lvavg, 1, MPI_DOUBLE, MPI_SUM,
		      MPI_COMM_WORLD);
	lvavg /= n;
...
void Timers::printStats(void)
{
	...
	if (i != BANDWIDTH) {
		...
		printf("%-17s\t%lf\t%lf\t%lf\t%lf\t%.1lf\n", lname, m_vavg[f],
		       m_vmin[f], m_vmax[f], m_elaps[f], percent);
	    } else {
		printf("%-17s\t%lf\t%lf\t%lf\n", lname, m_vavg[f], m_vmin[f],
		       m_vmax[f]);
	    }
	} else {
	    printf("%s\n", lname);
