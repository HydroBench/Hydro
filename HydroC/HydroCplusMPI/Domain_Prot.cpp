...
template < typename T >
    inline void lgrdstScal(const int f, const protectionMode_t mode, long &l,
			   T * x)
{
    int byt = 0;
    switch (mode) {
...
template < typename T >
    inline void lgrdstArr(const int f, const protectionMode_t mode, long &l,
			  T x)
{
    switch (mode) {
    case PROT_LENGTH:{
...
template < typename T >
    inline void lgrdstArrSimple(const int f, const protectionMode_t mode,
				long &l, T x, const int32_t lgr)
{
    int byt = 0;
    switch (mode) {
    case PROT_LENGTH:
...
void Domain::writeProtectionHeader(const int f)
{
	...
	if (m_myPe == 0) {
		...
		MPI_Irecv(&lgrs[i], 1, MPI_LONG, i, 987, MPI_COMM_WORLD,
			  &requests[reqcnt]);
		reqcnt++;
	    }
	} else {
	    MPI_Isend(&l, 1, MPI_LONG, 0, 987, MPI_COMM_WORLD,
		      &requests[reqcnt]);
	}
	...
	if (m_myPe == 0) {
	    for (int i = 1; i < m_nProc; i++) {
		MPI_Isend(&offsets[i], 1, MPI_LONG, i, 988, MPI_COMM_WORLD,
			  &requests[reqcnt]);
		reqcnt++;
	    }
	} else {
	    MPI_Irecv(&myOffset, 1, MPI_LONG, 0, 988, MPI_COMM_WORLD,
		      &requests[reqcnt]);
	    reqcnt++;
	}
	err = MPI_Waitall(reqcnt, requests, status);
