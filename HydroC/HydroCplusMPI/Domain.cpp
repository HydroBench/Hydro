...
Domain::Domain(int argc, char **argv)
{
	...
	if (m_myPe == 0 && m_stats > 0) {
	    cerr << "HydroC: allocated time too short " << tRemain << "s" <<
		endl;
	}
#ifdef MPI_ON
...
void Domain::setTiles()
{
...
#endif
	if (m_myPe == 0 && m_stats > 0)
	    cout << "Computing tilesize to " << tileSize
		<< " R=" << (float)m_nbtiles / (float)nTh << endl;
    } else {
	if (m_myPe == 0 && m_stats > 0)
    ...
    if (m_myPe == 0 && m_stats > 0)
	cout << "Forcing tilesize to " << tileSize << " at compile time" <<
	    endl;
#endif
...
#pragma omp parallel for private(i) if (m_numa) SCHEDULE
	...
	for (i = 0; i < m_nx; i += tileSize) {
	    ...
	    m_tiles[m_nbtiles++]->setExtend(tileSizeX, tileSizeY, m_nx, m_ny,
					    offx, offy, m_dx);
	    if (m_prt) {
