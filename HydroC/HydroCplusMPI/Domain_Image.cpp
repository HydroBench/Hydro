...
void Domain::getMaxVarValues(real_t * mxP, real_t * mxD, real_t * mxUV)
{
    ...
    iuvmax = sqrt(uuold(xmin, ymin) * uuold(xmin, ymin) +
		  vuold(xmin, ymin) * vuold(xmin, ymin));
	...
	for (x = xmin; x < xmax; x++) {
	    real_t v =
		sqrt(uuold(x, y) * uuold(x, y) + vuold(x, y) * vuold(x, y));
	    if (v > iuvmax)
		iuvmax = v;
	}
    }
...
void Domain::pngProcess(void)
{
    ...
    m_shrink = 1;
    while ((m_globNx / m_shrink) > m_shrinkSize ||
	   (m_globNy / m_shrink) > m_shrinkSize) {
	m_shrink++;
    }
	...
	for (cury = 0; cury < imgSizeY; cury++) {
		...
		*(ptr++) = (uint8_t) std::abs(255 * puold(x, y) / ipmax);
		*(ptr++) = (uint8_t) std::abs(255 * duold(x, y) / idmax);
		*(ptr++) =
		    (uint8_t) std::abs(255 *
				       sqrt(uuold(x, y) * uuold(x, y) +
					    vuold(x, y) * vuold(x,
								y)) / iuvmax);
	...
	for (cury = 0; cury < curImgY; cury++) {
		...
		x = xmin + curx * m_shrink;
		y = ymin + cury * m_shrink;
		*(ptr++) = (int)std::abs(255 * puold(x, y) / ipmax);
		*(ptr++) = (int)std::abs(255 * duold(x, y) / idmax);
		v1 = uuold(x, y) * uuold(x, y);
		v2 = vuold(x, y) * vuold(x, y);
		r = (int)std::abs(255 * sqrt(v1 + v2) / iuvmax);
		*(ptr++) = r;
		*(ptr++) = 255;	//
	    }
	}
	...
	for (d = 1; d < m_nProc; d++) {
	    ...
	    // send our sub image
	    size =
		((xmax - xmin) / m_shrink) * ((ymax -
					       ymin) / m_shrink) * PIXRGBA;
	    if (m_myPe == 0) {
		// determine the box of the distant domain
		CalcSubSurface(0, m_globNx, 0, m_globNy, 0, m_nProc - 1, itsbox,
			       d);
		...
		MPI_Irecv(bufferR, size * sizeof(*bufferR), MPI_CHAR, d,
			  987 + d, MPI_COMM_WORLD, &request[0]);
		nbreq++;
	    } else {
		if (m_myPe == d) {
		    // cerr << m_myPe << " Isend " << endl;
		    MPI_Isend(m_buffer, size * sizeof(*m_buffer), MPI_CHAR, 0,
			      987 + d, MPI_COMM_WORLD, &request[0]);
		    nbreq++;
		}
	    }
		...
		for (cury = 0; cury < DimgSizeY; cury++) {
		    ...
		    dest += (xmin / m_shrink);
		    dest *= PIXRGBA;
		    memcpy(&m_buffer[dest], &bufferR[src],
			   lgr * sizeof(*bufferR));
		    src += lgr;
		}
		delete[]bufferR;
	...
	if ((m_shrink > 1) && (m_myPe == 0)) {
		...
		for (curx = 0; curx < imgSizeX; curx++) {
			...
			for (int vy = -extrem; vy < extrem + 1; vy++) {
				...
				pngFillGap(curx, cury, curx + vx, cury + vy,
					   Iptr, cpt, imgSizeX, imgSizeY);
			    }
			}
			if (cpt) {
			    for (int c = 0; c < PIXRGBA; c++) {
	...
	for (cury = 0; cury < imgSizeY; cury++) {
		...
		ptr[0] = (png_byte) std::abs(255 * puold(x, y) / ipmax);
		ptr[1] = (png_byte) std::abs(255 * duold(x, y) / idmax);
		ptr[2] =
		    (png_byte) std::abs(255 *
					sqrt(uuold(x, y) * uuold(x, y) +
					     vuold(x, y) * vuold(x,
								 y)) / iuvmax);
		ptr[3] = 255;
	...
	for (cury = 0; cury < curImgY; cury++) {
		...
		x = xmin + curx * m_shrink;
		y = ymin + cury * m_shrink;
		ptr[0] = (int)std::abs(255 * puold(x, y) / ipmax);
		ptr[1] = (int)std::abs(255 * duold(x, y) / idmax);
		v1 = uuold(x, y) * uuold(x, y);
		v2 = vuold(x, y) * vuold(x, y);
		r = (int)std::abs(255 * sqrt(v1 + v2) / iuvmax);
		ptr[2] = r;
	...
	for (d = 1; d < m_nProc; d++) {
		...
		assert(m_buffer != 0);
		MPI_Irecv(m_buffer, size, MPI_CHAR, d, 987 + d, MPI_COMM_WORLD,
			  &request[0]);
		nbreq++;
	    } else {
		if (m_myPe == d) {
		    // cerr << m_myPe << " Isend " << endl;
		    MPI_Isend(m_buffer, size, MPI_CHAR, 0, 987 + d,
...
void Domain::pngFillGap(int curx, int cury, int nx, int ny, int Iptr[4],
			int &cpt, int32_t imgSizeX, int32_t imgSizeY)
{
#if WITHPNG > 0
    png_byte *nrow, *nptr;
...
#define BINARY 1
	...
	png_set_IHDR(m_png_ptr, m_info_ptr, imgSizeX, imgSizeY, bit_depth,
