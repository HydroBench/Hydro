...
{
	uint32_t xmin, xmax, ymin, ymax, x, y;
	real_t ipmax = 0;
	real_t idmax = 0;
	real_t iuvmax = 0;
	...
	for (y = ymin; y < ymax; y++) {
		for (x = xmin; x < xmax; x++) {
			...
			if (v > iuvmax) iuvmax = v;
		}
	}
	*mxP = ipmax; *mxD = idmax; *mxUV = iuvmax;
}
...
void Domain::pngProcess(void)
{
	...
	*/
	...
	if (m_nProc == 1) {
		...
		getExtends(TILE_INTERIOR, xmin, xmax, ymin, ymax);
		uint32_t curImgX = (xmax - xmin) / m_shrink;
		uint32_t curImgY = (ymax - ymin) / m_shrink;
		uint32_t cury = 0;
		uint32_t curx = 0;
		...
		for (d = 1; d < m_nProc; d++) {
			...
			if (m_myPe == 0) {
				uint32_t lcurx, lgr;
				uint32_t DimgSizeX = ((xmax - xmin) / m_shrink);
				uint32_t DimgSizeY = ((ymax - ymin) / m_shrink);
				int src = 0, dest = 0;
				curx = 0;
				cury = 0;
			...
			for (cury = 0; cury < imgSizeY; cury++) {
				for (curx = 0; curx < imgSizeX; curx++) {
					...
					if (ptr[3] < 255) {
						...
						int Iptr[PIXRGBA];
						int extrem = 1;
						for (int c=0; c < PIXRGBA; c++) Iptr[c] = 0;
...
void Domain::pngFillGap(int curx, int cury, int nx, int ny, int Iptr[4], int &cpt, uint32_t imgSizeX, uint32_t imgSizeY) {
#if WITHPNG > 0   
	png_byte* nrow, *nptr;
...
#define BINARY 1
	...
	if (m_myPe == 0) {
		...
		png_set_IHDR(m_png_ptr, m_info_ptr, imgSizeX, imgSizeY,
