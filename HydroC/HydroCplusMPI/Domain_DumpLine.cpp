...
void Domain::dumpOneArray(FILE * f, Matrix2 < real_t > &p)
{
	...
	if ((itsbox[YMIN_D] <= y) && (y < itsbox[YMAX_D])) {
	    if (m_myPe == 0) {
		MPI_Recv(&nd[d], 1, MPI_INT, d, 887 + 2 * d, MPI_COMM_WORLD, status);	//, &request[0]);
	    } else {
		if (m_myPe == d) {
	...
	if ((itsbox[YMIN_D] <= y) && (y < itsbox[YMAX_D])) {
	    if (m_myPe == 0) {
		MPI_Recv(&gbuf[itsbox[XMIN_D]], nd[d], MPI_DOUBLE, d,
			 887 + 2 * d + 1, MPI_COMM_WORLD, status);
	    } else {
		if (m_myPe == d) {
		    MPI_Send(buf, nd[d], MPI_DOUBLE, 0, 887 + 2 * d + 1,
			     MPI_COMM_WORLD);
		}
	    }
	}			// if y is inside
...
void Domain::dumpLineArray(FILE * f, Matrix2 < real_t > &p, char *name,
			   char *ext)
{
    char fname[256];
    if (m_myPe == 0) {
