    ...
    if (m_nProc == 1) {
        ...
        for (cury = 0; cury < imgSizeY; cury++) {
            for (curx = 0; curx < imgSizeX; curx++) {
                ...
                *(ptr++) = (uint8_t)std::abs(
                    255 * sqrt(uuold(x, y) * uuold(x, y) + vuold(x, y) * vuold(x, y)) / iuvmax);
        ...
        for (d = 1; d < m_nProc; d++) {
            ...
            if (m_myPe == 0) {
                ...
                MPI_Irecv(bufferR, size * sizeof(*bufferR), MPI_CHAR, d, 987 + d, MPI_COMM_WORLD,
                          &request[0]);
                nbreq++;
            } else {
                if (m_myPe == d) {
                    ...
                    if (ptr[3] < 255) {
                        ...
                        for (int vy = -extrem; vy < extrem + 1; vy++) {
                            for (int vx = -extrem; vx < extrem + 1; vx++) {
                                pngFillGap(curx, cury, curx + vx, cury + vy, Iptr, cpt, imgSizeX,
                                           imgSizeY);
                            }
                        }
                        if (cpt) {
...
void Domain::pngFillGap(int curx, int cury, int nx, int ny, int Iptr[4], int &cpt, int32_t imgSizeX,
                        int32_t imgSizeY) {
#if WITHPNG > 0
    png_byte *nrow, *nptr;
