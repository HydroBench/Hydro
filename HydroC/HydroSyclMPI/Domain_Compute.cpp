...
real_t Domain::computeTimeStep() {
	...
	for (int32_t pass = 0; pass < 2; pass++) {
		...
		double endT = Custom_Timer::dcclock();
		...
		if (m_prt) {
			std::cout << "After pass " << pass << " direction [" << m_scan
					<< "]" << std::endl;
		}
...
void Domain::compute() {
	...
	while (m_tcur < m_tend) {
...

		{
			...
			if (needToStopGlob) {
				if (myPe == 0) {
					std::cerr << " Hydro stops by time limit "
							<< m_tr.timeRemain() << " < " << m_timeGuard
							<< std::endl;
				}
				std::cout.flush();
				std::cerr.flush();
	...
	if (m_checkPoint) {
		...
		if (myPe == 0) {
			...
			double elaps = (end - start);
			Custom_Timer::convertToHuman(txt, elaps);
			std::cerr << "Write protection in " << txt << " (" << elaps << "s)"
					<< std::endl;
		}
	}
	...
	if (myPe == 0) {
		...
		if (ParallelInfo::nb_procs() > 1) {
			printf(" maxMatrixTot %.3f GB",
					float(
							Matrix2<double>::getMax() * ParallelInfo::nb_procs()
									/ giga));
		}
		printf("\n");
		...
		ecartCellPerSec = sqrt(
				(ecartCellPerSec / nbTotCelSec)
