...
void Tile::setExtend(int32_t nx, int32_t ny, int32_t gnx, int32_t gny,
		int32_t offx, int32_t offy, int32_t Extralayer) {
	m_nx = nx;
	m_ny = ny;
	m_gnx = gnx;
	m_gny = gny;
...
void Tile::trace(int32_t row, int32_t col, real_t zerol, real_t zeror,
		real_t project, real_t dtdx) {
	int32_t xmin, xmax, ymin, ymax;
	getExtends(TILE_FULL, xmin, xmax, ymin, ymax);
	...
	if (row >= ymin && row < ymax && col >= xmin && col < xmax) {
		...
		traceonRow(col, col + 1, dtdx, zeror, zerol, project, cS, qIDS, qIUS,
...
void Tile::traceonRow(int32_t xmin, int32_t xmax, real_t dtdx, real_t zeror,
		real_t zerol, real_t project, Preal_t cS, Preal_t qIDS, Preal_t qIUS,
		Preal_t qIVS, Preal_t qIPS, Preal_t dqIDS, Preal_t dqIUS, Preal_t dqIVS,
		Preal_t dqIPS, Preal_t pqxpIDS, Preal_t pqxpIUS, Preal_t pqxpIVS,
		Preal_t pqxpIPS, Preal_t pqxmIDS, Preal_t pqxmIUS, Preal_t pqxmIVS,
		Preal_t pqxmIPS) {
...
void Tile::compflxOnRow(int32_t xmin, int32_t xmax, real_t entho,
		Preal_t qgdnvIDS, Preal_t qgdnvIUS, Preal_t qgdnvIVS, Preal_t qgdnvIPS,
	...
	for (int32_t i = xmin; i < xmax; i++) {
		...
		// Total energy
		real_t ekin = my_half * qgdnvIDS[i]
				* (Square(qgdnvIUS[i]) + Square(qgdnvIVS[i]));
...
void Tile::compflx(int32_t y, int32_t x) {
	...
	if (y >= ymin && y < ymax && x >= xmin && x < xmax) {
		...
		compflxOnRow(x, x + 1, entho, qgdnvIDS, qgdnvIUS, qgdnvIVS, qgdnvIPS,
				fluxIVS, fluxIUS, fluxIPS, fluxIDS);
	}
}

void Tile::updateconservXscan(int32_t xmin, int32_t xmax, Preal_t uIDS,
		Preal_t uIUS, Preal_t uIVS, Preal_t uIPS, Preal_t uoldIDS,
		Preal_t uoldIUS, Preal_t uoldIVS, Preal_t uoldIPS) {

	for (int32_t i = xmin; i < xmax; i++) {
		uoldIDS[i + m_offx] = uIDS[i];
	}
	for (int32_t i = xmin; i < xmax; i++) {
		uoldIVS[i + m_offx] = uIVS[i];
	}
	for (int32_t i = xmin; i < xmax; i++) {
		uoldIUS[i + m_offx] = uIUS[i];
	}
	for (int32_t i = xmin; i < xmax; i++) {
		uoldIPS[i + m_offx] = uIPS[i];
	}
}

void Tile::updateconserv1Row(int32_t xmin, int32_t xmax, real_t dtdx,
		Preal_t uIDS, Preal_t uIUS, Preal_t uIVS, Preal_t uIPS, Preal_t uoldIDS,
		Preal_t uoldIUS, Preal_t uoldIVS, Preal_t uoldIPS, Preal_t fluxIDS,
		Preal_t fluxIVS, Preal_t fluxIUS, Preal_t fluxIPS) {

	for (int32_t i = xmin; i < xmax; i++) {
		uIDS[i] = uIDS[i] + (fluxIDS[i - 2] - fluxIDS[i - 1]) * dtdx;
	}
	for (int32_t i = xmin; i < xmax; i++) {

		uIVS[i] = uIVS[i] + (fluxIVS[i - 2] - fluxIVS[i - 1]) * dtdx;
	}
	for (int32_t i = xmin; i < xmax; i++) {

		uIUS[i] = uIUS[i] + (fluxIUS[i - 2] - fluxIUS[i - 1]) * dtdx;
	}
	for (int32_t i = xmin; i < xmax; i++) {
		uIPS[i] = uIPS[i] + (fluxIPS[i - 2] - fluxIPS[i - 1]) * dtdx;
	}
}

void Tile::updateconservYscan(int32_t x, int32_t ymin, int32_t ymax,
		Preal_t uoldIDS, Preal_t uoldIPS, Preal_t uoldIVS, Preal_t uoldIUS,
		RArray2D<real_t> &uID, RArray2D<real_t> &uIP, RArray2D<real_t> &uIV,
		RArray2D<real_t> &uIU,
	...
	for (int32_t y = ymin; y < ymax; y++) {
		uoldIUS[y + m_offx] = ((real_t*__restrict__) uIV[y])[x];
	}

	for (int32_t y = ymin; y < ymax; y++) {
		uoldIPS[y + m_offx] = ((real_t*__restrict__) uIP[y])[x];
	}
}
...
void Tile::updateconserv() {
	...
	if (m_scan == X_SCAN) {
		for (int32_t y = ymin; y < ymax; y++) {
			...
			updateconservXscan(xmin, xmax, uIDS, uIUS, uIVS, uIPS, uoldIDS,
					uoldIUS, uoldIVS, uoldIPS);
		}
	} else {
		for (int32_t x = xmin; x < xmax; x++) {
			auto uoldIDS = uoldID.getRow(x + m_offy);
			auto uoldIPS = uoldIP.getRow(x + m_offy);
			auto uoldIVS = uoldIV.getRow(x + m_offy);
			auto uoldIUS = uoldIU.getRow(x + m_offy);
			...
			updateconservYscan(x, ymin, ymax, uoldIDS, uoldIPS, uoldIVS,
					uoldIUS, uID, uIP, uIV, uIU, pl);
		}
	}

} // updateconserv

void Tile::updateconserv1(int32_t y, int32_t x, real_t dtdx) {
	int32_t xmin, xmax, ymin, ymax;
	getExtends(TILE_INTERIOR, xmin, xmax, ymin, ymax);
	if (y >= ymin && y < ymax && x >= xmin && x < xmax) {
		auto uoldID = (deviceSharedVariables()->m_uold)(ID_VAR);
		...
		Preal_t fluxIUS = fluxIU.getRow(y);
		Preal_t fluxIPS = fluxIP.getRow(y);
		updateconserv1Row(x, x + 1, dtdx, uIDS, uIUS, uIVS, uIPS, uoldIDS,
				uoldIUS, uoldIVS, uoldIPS, fluxIDS, fluxIVS, fluxIUS, fluxIPS);
	}
}

void Tile::updateconserv1() {
	int32_t xmin, xmax, ymin, ymax;
	getExtends(TILE_INTERIOR, xmin, xmax, ymin, ymax);
	real_t dtdx = m_onDevice->m_dt / m_onDevice->m_dx;
	for (int32_t y = ymin; y < ymax; y++)
	...
	if (x >= xmin && x < xmax && y >= ymin && y < ymax) {
		...
		if (m_scan == X_SCAN) {
			uoldID[y + m_offy][x + m_offx] = uID[y][x]
					+ (fluxID[y][x - 2] - fluxID[y][x - 1]) * dtdx;
			uoldIP[y + m_offy][x + m_offx] = uIP[y][x]
					+ (fluxIP[y][x - 2] - fluxIP[y][x - 1]) * dtdx;
			uoldIU[y + m_offy][x + m_offx] = uIU[y][x]
					+ (fluxIU[y][x - 2] - fluxIU[y][x - 1]) * dtdx;
			uoldIV[y + m_offy][x + m_offx] = uIV[y][x]
					+ (fluxIV[y][x - 2] - fluxIV[y][x - 1]) * dtdx;
		} else {
...
void Tile::eos(tileSpan_t span) {
	...
	real_t smallp = Square(deviceSharedVariables()->m_smallc)
...
void Tile::compute_dt_loop1OnRow(int32_t xmin, int32_t xmax, Preal_t qIDS,
		Preal_t qIPS, Preal_t qIUS, Preal_t qIVS, Preal_t uoldIDS,
...
void Tile::compute_dt_loop2OnRow(real_t &tmp1, real_t &tmp2, int32_t xmin,
		int32_t xmax, Preal_t cS, Preal_t qIUS, Preal_t qIVS) {
	for (int32_t i = xmin; i < xmax; i++) {
...
void Tile::constprimOnRow(int32_t xmin, int32_t xmax, Preal_t qIDS,
		Preal_t qIPS, Preal_t qIVS, Preal_t qIUS, const Preal_t uIDS,
		const Preal_t uIPS, const Preal_t uIVS, const Preal_t uIUS,
		Preal_t eS) {
...
void Tile::constprim(int32_t row, int32_t col) {
	...
	if (row >= ymin && row < ymax && col >= xmin && col < xmax) {
		...
		constprimOnRow(col, col + 1, qIDS, qIPS, qIVS, qIUS, uIDS, uIPS, uIVS,
				uIUS, eS);
	}
...
void Tile::riemannOnRowInRegs(int32_t xmin, int32_t xmax, real_t smallp,
		real_t gamma6, real_t smallpp, Preal_t qgdnvIDS, Preal_t qgdnvIUS,
		Preal_t qgdnvIPS, Preal_t qgdnvIVS, Preal_t qleftIDS, Preal_t qleftIUS,
...
#pragma omp simd
	for (int32_t i = xmin; i < xmax; i++) {
		...
		pstarI = sycl::max(
				((wr_i * plI + wl_i * prI) + wl_i * wr_i * (ulI - urI))
						/ (wl_i + wr_i), zero);
		//  #pragma ivdep
		for (int32_t iter = 0; iter < 10; iter++) {
			if (go_onI) {
				...
				real_t wwl = sycl::sqrt(
						clI * (one + gamma6 * (pst - plI) / plI));
				real_t wwr = sycl::sqrt(
						crI * (one + gamma6 * (pst - prI) / prI));
		...
		real_t ustar_i = my_half
				* (ulI + (plI - pstarI) / wl_i + urI - (prI - pstarI) / wr_i);
...
void Tile::riemann() {
	...
	real_t smallp = Square(deviceSharedVariables()->m_smallc)
			/ deviceSharedVariables()->m_gamma;
	real_t gamma6 = (deviceSharedVariables()->m_gamma + one)
			/ (two * deviceSharedVariables()->m_gamma);
	...
	for (int32_t s = ymin; s < ymax; s++) {
		...
		riemannOnRowInRegs(xmin, xmax, smallp, gamma6, smallpp, qgdnvIDS,
				qgdnvIUS, qgdnvIPS, qgdnvIVS, qleftIDS, qleftIUS, qleftIPS,
				qleftIVS, qrightIDS, qrightIUS, qrightIPS, qrightIVS,
				getSGNM());
	}

} // riemann

void Tile::riemann(int32_t row, int32_t col, real_t smallp, real_t gamma6,
		real_t smallpp) {
	int32_t xmin, xmax, ymin, ymax;
	getExtends(TILE_FULL, xmin, xmax, ymin, ymax);
	...
	if (row >= ymin && row < ymax && col >= xmin && col < xmax) {
		...
		riemannOnRowInRegs(col, col + 1, smallp, gamma6, smallpp, qgdnvIDS,
				qgdnvIUS, qgdnvIPS, qgdnvIVS, qleftIDS, qleftIUS, qleftIPS,
				qleftIVS, qrightIDS, qrightIUS, qrightIPS, qrightIVS,
				getSGNM());
	}
...
void Tile::boundary_process(int32_t boundary_left, int32_t boundary_right,
		int32_t boundary_up, int32_t boundary_down) {
	...
	if (m_scan == Y_SCAN) {
		...
		if (boundary_up > 0) {
			for (int32_t ivar = 0; ivar < NB_VAR; ivar++) {
				...
				int32_t j_max = sycl::min(ymax + m_offy,
						m_gny + 2 * m_extraLayer);
				for (int32_t j = j_min; j < j_max; j++) {
					real_t sign = 1;
					int32_t j0;
					if (boundary_up == 1) {
