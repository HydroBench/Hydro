...
void
Tile::slopeOnRow (int32_t xmin, int32_t xmax, Preal_t qS, Preal_t dqS,
...
void
Tile::slope (int32_t y, int32_t x, real_t ov_slope_type)
{
  int32_t xmin, xmax, ymin, ymax;
  getExtends (TILE_FULL, xmin, xmax, ymin, ymax);
...
void
Tile::trace (int32_t row, int32_t col, real_t zerol, real_t zeror,
...
Tile::trace ()
{
  ...
  if (deviceSharedVariables ()->m_scheme == SCHEME_PLMDE)
    { // standard PLMDE
      zerol = zero;
      zeror = zero;
      project = one;
  ...
  if (deviceSharedVariables ()->m_scheme == SCHEME_COLLELA)
    { // Collela's method
      zerol = zero;
      zeror = zero;
      project = zero;
  ...
  for (int32_t s = ymin; s < ymax; s++)
    {
      ...
      traceonRow (xmin, xmax, dtdx, zeror, zerol, project, cS, qIDS, qIUS, qIVS,
		  qIPS, dqIDS, dqIUS, dqIVS, dqIPS, pqxpIDS, pqxpIUS, pqxpIVS,
		  pqxpIPS, pqxmIDS, pqxmIUS, pqxmIVS, pqxmIPS);
    }

} // trace

void
Tile::traceonRow (int32_t xmin, int32_t xmax, real_t dtdx, real_t zeror,
		  real_t zerol, real_t project, Preal_t cS, Preal_t qIDS,
		  Preal_t qIUS, Preal_t qIVS, Preal_t qIPS, Preal_t dqIDS,
		  Preal_t dqIUS, Preal_t dqIVS, Preal_t dqIPS, Preal_t pqxpIDS,
		  Preal_t pqxpIUS, Preal_t pqxpIVS, Preal_t pqxpIPS,
		  Preal_t pqxmIDS, Preal_t pqxmIUS, Preal_t pqxmIVS,
		  Preal_t pqxmIPS)
{
...
void
Tile::qleftrOnRow (int32_t xmin, int32_t xmax, Preal_t pqleftS,
...
void
Tile::qleftr (int32_t y, int32_t x)
{
  int32_t xmin, xmax, ymin, ymax;
...
void
Tile::compflxOnRow (int32_t xmin, int32_t xmax, real_t entho, Preal_t qgdnvIDS,
		    Preal_t qgdnvIUS, Preal_t qgdnvIVS, Preal_t qgdnvIPS,
...
void
Tile::compflx (int32_t y, int32_t x)
{
  int32_t xmin, xmax, ymin, ymax;
  getExtends (TILE_FULL, xmin, xmax, ymin, ymax);
...
void
Tile::updateconservXscan (int32_t xmin, int32_t xmax, real_t dtdx, Preal_t uIDS,
			  Preal_t uIUS, Preal_t uIVS, Preal_t uIPS,
			  Preal_t uoldIDS, Preal_t uoldIUS, Preal_t uoldIVS,
			  Preal_t uoldIPS, Preal_t fluxIDS, Preal_t fluxIVS,
...
Tile::gatherconserv (int32_t d)
{
  ...
  if ((m_scan == X_SCAN && d >= ymin && d < ymax)
	...
	{
	  uID.putFullCol (d, 0, uoldID.getRow (d + m_offy) + m_offx,
			  (ymax - ymin));
	  uIU.putFullCol (d, 0, uoldIV.getRow (d + m_offy) + m_offx,
			  (ymax - ymin));
	  uIV.putFullCol (d, 0, uoldIU.getRow (d + m_offy) + m_offx,
			  (ymax - ymin));
	  uIP.putFullCol (d, 0, uoldIP.getRow (d + m_offy) + m_offx,
			  (ymax - ymin));
	}
...
void
Tile::eosOnRow (int32_t xmin, int32_t xmax, real_t smallp, Preal_t qIDS,
...
void
Tile::eos (tileSpan_t span, int32_t y, int32_t x, real_t smallp)
{
  int32_t xmin, xmax, ymin, ymax;
  getExtends (span, xmin, xmax, ymin, ymax);
  if (y >= ymin && y < ymax && x >= xmin && x < xmax)
...
void
Tile::compute_dt_loop1OnRow (int32_t xmin, int32_t xmax, Preal_t qIDS,
...
void
Tile::compute_dt_loop2OnRow (real_t &tmp1, real_t &tmp2, int32_t xmin,
...
Tile::constprimOnRow (int32_t xmin, int32_t xmax, Preal_t qIDS, Preal_t qIPS,
		      Preal_t qIVS, Preal_t qIUS, const Preal_t uIDS,
		      const Preal_t uIPS, const Preal_t uIVS,
...
void
Tile::constprim (int32_t row, int32_t col)
{
...
void
Tile::riemannOnRowInRegs (int32_t xmin, int32_t xmax, real_t smallp,
...
#pragma omp simd
  for (int32_t i = xmin; i < xmax; i++)
      ...
      if (pstarI >= po_i)
	{
      ...
      real_t scr_i = sycl::max (
	  (real_t) (spout_i - spin_i),
...
void
Tile::riemann (int32_t row, int32_t col, real_t smallp, real_t gamma6,
	       real_t smallpp)
{
  int32_t xmin, xmax, ymin, ymax;
  getExtends (TILE_FULL, xmin, xmax, ymin, ymax);
...
void
Tile::boundary_process (int32_t boundary_left, int32_t boundary_right,
	  ...
	  for (int32_t ivar = 0; ivar < NB_VAR; ivar++)
	    {
	      ...
	      int32_t i_max = sycl::min (xmax + m_offx,
					 m_gnx + 2 * m_ExtraLayer);
